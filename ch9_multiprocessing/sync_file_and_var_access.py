import multiprocessing
import os

import fasteners as fasteners

MAX_COUNT_PER_PROCESS = 1000
FILENAME = "count.txt"


@fasteners.interprocess_locked('/tmp/tmp_lock')
def work(filename, max_count):
    for n in range(max_count):
        f = open(filename, "r")
        try:
            nbr = int(f.read())
        except ValueError as err:
            print(
                "File is empty, starting to count from 0, error: " + str(err))
            nbr = 0
        f = open(filename, "w")
        f.write(str(nbr + 1) + '\n')
        f.close()


def work_2(value, max_count, lock):
    for n in range(max_count):
        with lock:
            value.value += 1


def run_workers():
    NBR_PROCESSES = 4
    total_expected_count = NBR_PROCESSES * MAX_COUNT_PER_PROCESS
    print("Starting {} process(es) to count to {}".format(NBR_PROCESSES,
                                                          total_expected_count))
    # reset counter
    f = open(FILENAME, "w")
    f.close()

    lock = multiprocessing.Lock()
    value = multiprocessing.Value('i', 0)

    processes = []
    for process_nbr in range(NBR_PROCESSES):
        # p = multiprocessing.Process(target=work, args=(FILENAME,
        #                                                MAX_COUNT_PER_PROCESS))
        p = multiprocessing.Process(target=work_2,
                                    args=(
                                        value, MAX_COUNT_PER_PROCESS, lock))

        p.start()
        processes.append(p)

    for p in processes:
        p.join()

    print("Expecting to see a count of {}".format(total_expected_count))
    print("{} contains:".format(FILENAME))
    os.system('more ' + FILENAME)


if __name__ == "__main__":
    run_workers()
